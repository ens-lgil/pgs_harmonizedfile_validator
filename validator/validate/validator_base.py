...
class ValidatorBase:
    ...
    def validate_data(self):
        ...
        for chunk in self.df_iterator(self.file):
            ...
            # Schema validation
            self.schema = Schema([self.validators[h] for h in self.cols_to_validate])                
    ...
    def check_build_is_legit(self,build):
        if build in BUILD_LIST:
            return True
        return False

    
    def get_comments_info(self,type):
